    ...
    public CompletionTask createTask(int queryType, JTextComponent jtc)
    {
        ...
        return new AsyncCompletionTask(new AsyncCompletionQuery()
        {
            ...
            protected void query(CompletionResultSet completionResultSet, Document document, int caretOffset)
            {
                ...
                int lineStartOffset;
                String filter;
                int startOffset = caretOffset - 1; // the beginning of a complementary word
                String currentLine;
                String text;
                ...
                try
                {
                    ...
                    lineStartOffset = getRowFirstNonWhite(bDoc, caretOffset); // the beginning of the current line
                    currentLine = getCurrentLine(bDoc, caretOffset);
                    //System.out.println(currentLine);
                    final char[] line = bDoc.getText(lineStartOffset, caretOffset - lineStartOffset).toCharArray();
                    currentLine = String.valueOf(line); // Text from beginning of line to cursor
                    //System.out.println(currentLine);
                    final int whiteOffset = indexOfWhite(line); // last space
                    filter = new String(line, whiteOffset + 1, line.length - whiteOffset - 1); // a completed word
                    if (whiteOffset > 0)
                    {
                ...
                else if (currentLine.matches(".*(<|\\[)@\\w*"))
                {
                    ...
                    Pattern pattern = Pattern.compile("(<|\\[)#macro\\s+(\\w+)");
                    ...
                    {
                        completionResultSet.addItem( FTLCompletionItem.macro( name, caretOffset - filter.length(), caretOffset));
                    }
                }
